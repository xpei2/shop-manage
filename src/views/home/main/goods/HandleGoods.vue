    ...
    methods: {
        ...
        cascaderCateChanged() {
            ...
            if (this.addGoodsForm.goods_cat.length !== 3) {
                this.$message.warning("只允许选择三级分类！");
                // 清空级联选择器数据
        ...
        beforeTabLeave(activeName, oldActiveName) {
            if (
                ...
                this.$message.error("请检查是否选择了商品分类！");
                return false;
            }
        },
        ...
        beforeUpload(file) {
            if (parseInt(file.size / 1024) > 500) {
                this.$message.error("上传失败，图片超过规定大小！");
                return false;
            } else if (!imageRegExp(file.type)) {
                this.$message.error(
                    "上传失败，只允许上传jpg/png/jpeg类型的图片！"
                );
                return false;
            }
        },
        ...
        addSubmit() {
            this.$refs.addGoodsRef.validate(valid => {
                if (!valid) {
                    return this.$message.error("请检查必填项是否都填写完成！");
                } else {
                    // 深拷贝处理分类
                    const form = _.cloneDeep(this.addGoodsForm);
                    // 判断分类参数是否是数组，如果是则转换为字符串
                    form.goods_cat =
                        Object.prototype.toString.call(form.goods_cat) ===
                        "[object Array]"
                            ? form.goods_cat.join(",")
                            : form.goods_cat;
                    // 清空参数列表并处理动态参数
        ...
        async postAddGoods(obj) {
            await postAddGoods(obj).then(res => {
                ...
                if (addGoodsRes.meta.status !== 201) {
                    return this.$message.error(addGoodsRes.meta.msg);
                } else {
                    this.$message.success(addGoodsRes.meta.msg);
                    this.isSubmit = true;
                }
            });
        },
        ...
        async getGoodsInfo() {
            await getGoodsInfo(this.goodsId).then(res => {
                ...
                if (getGoodsRes.meta.status !== 200) {
                    return this.$message.error(getGoodsRes.meta.msg);
                } else {
                    this.$message.success(getGoodsRes.meta.msg);
                    // 处理参数信息
        ...
        async putGoodsEdit(obj) {
            await putGoodsEdit(this.goodsId, obj).then(res => {
                ...
                if (editGoodsRes.meta.status !== 200) {
                    return this.$message.error(editGoodsRes.meta.msg);
                } else {
        ...
        async getCateList() {
            await getCateData().then(res => {
                ...
                if (cateRes.meta.status !== 200) {
                    return this.$message.error(cateRes.meta.msg);
                } else {
        ...
        async getParamsList(paramsSel) {
            await getParamsData(this.getCateId, paramsSel).then(res => {
                ...
                if (paramsRes.meta.status !== 200) {
                    return this.$message.error("获取默认参数失败！");
                } else {
                    switch (paramsSel) {
                        case "many":
