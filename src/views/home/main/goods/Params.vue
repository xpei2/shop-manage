<template>
    <div id="params">
        ...
        <el-card>
            ...
            >
                <params-expand
                    @addParamsAttr="addParamsAttr"
                    @removeParamsAttr="removeParamsAttr"
                />
            </params-table>
        </el-card>
        <!-- 添加/编辑参数对话框公用 -->
        ...
        >
            <el-form-item :label="paramsDialogInfo.labelName" prop="attr_name">
...
export default {
    ...
    methods: {
        ...
        async postAddParams(cateId, obj) {
            await postAddParams(cateId, obj).then(res => {
                ...
                if (addParamsRes.meta.status !== 201) {
                    return this.$message.error(addParamsRes.meta.msg);
                } else {
                    this.$message.success(addParamsRes.meta.msg);
                    // 对话框隐藏
                    this.dialogClose();
                    // 重新获取数据
                    this.getParamsList();
        ...
        async getParamsInfo(cateId, attrId, sel) {
            await getParamsInfo(cateId, attrId, sel).then(res => {
                ...
                if (getParamsRes.meta.status !== 200) {
                    return this.$message.error(getParamsRes.meta.msg);
                } else {
                    this.$message.success(getParamsRes.meta.msg);
        ...
        async putParamsEdit(cateId, attrId, obj, isRefresh = true) {
            await putParamsEdit(cateId, attrId, obj).then(res => {
                const editCateRes = res.data;
                if (editCateRes.meta.status !== 200) {
                    return this.$message.error(editCateRes.meta.msg);
                } else {
                    // 提示修改成功
                    this.$message.success(editCateRes.meta.msg);
                    if (isRefresh) {
                        // 对话框隐藏
                        this.dialogClose();
                        // 重新获取数据
        ...
        async deleteParams(cateId, attrId) {
            await this.$confirm("是否永久删除该参数信息/属性信息？", "提示", {
