                ...
                <el-table-column label="操作" width="320">
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button
                            ...
                            icon="el-icon-delete"
                            size="mini"
                            @click="removeClick(scope.row.cat_id)"
                            >删除</el-button
                        >
                    </template>
                </el-table-column>
    ...
    methods: {
        ...
        cascaderCateChanged() {
            ...
            if (this.goods_cat.length > 0) {
                this.cateFormModel.cat_pid = this.goods_cat[
                    this.goods_cat.length - 1
                ];
        ...
        async postAddCate(obj) {
            await postAddCate(obj).then(res => {
                ...
                if (addCateRes.meta.status !== 201) {
                    return this.$message.error(addCateRes.meta.msg);
                } else {
                    this.$message.success(addCateRes.meta.msg);
                    // 关闭对话框
                    this.dialogClose();
                    // 重新获取数据
                    this.getCateList();
        ...
        async getCateInfo(cateId) {
            await getCateInfo(cateId).then(res => {
                ...
                if (getCateRes.meta.status !== 200) {
                    return this.$message.error(getCateRes.meta.msg);
                } else {
                    this.$message.success(getCateRes.meta.msg);
        ...
        async putCateEdit(cateId, cateName) {
            await putCateEdit(cateId, cateName).then(res => {
